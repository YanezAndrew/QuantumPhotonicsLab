...
x_step_range = x_max/increment
y_step_range = y_max/increment
ROI_y_origin = 98
ROI_y_height = 17
...
# initialize the communication with the Newport Controller and set it to (0,0)
# initialize the pl_plot
# ----------------------------------------------------- START
device_ip = '192.168.254.254'
device_port = 5001
esp = ESP(device_ip, device_port) # connects the device via ethernet
axis_x = esp.axis(1) # x
axis_y = esp.axis(2) # y
print("Moving in the increment of", increment, "mm with interval times of", t_exp, "seconds")
axis_x.move_to(0)
axis_y.move_to(0)
...
for v_x in np.arange(0, x_max, increment):
    x= int(v_x/v_res)
    Set_X(hdl, v_x)
    time.sleep(0.3)
    Read_X(hdl)
    Read_Y(hdl)
    if v_y == 0:
        for v_y in np.arange(0, x_max, increment):
            y = int(v_y/v_res)
            Set_Y(hdl, v_y)
            time.sleep(0.3)
            Read_X(hdl)
